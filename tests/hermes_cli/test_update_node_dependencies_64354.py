...
def _fake_hermes_constants_module(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fake_with_hermes_node_path(env):
        # The real helper merges env over os.environ; pass through whatever the
        # caller hands us (including empty dict for nixos_env).
        return dict(env or {})

    stub.with_hermes_node_path = fake_with_hermes_node_path
    stub.get_default_hermes_root = lambda: str(Path("/tmp/hermes_root_stub"))
    monkeypatch.setitem(sys.modules, "hermes_constants", stub)


# ``_update_node_dependencies`` calls ``_nixos_build_env()`` from hermes_cli.main
# (which returns None on non-NixOS systems). The real call is done at *runtime*
# against the reloaded module, so we expose a fixture-level helper to monkeypatch
# the symbol for tests that drive the function end-to-end.
def _stub_nixos_env(monkeypatch, pkg_main, env=None) -> None:
    monkeypatch.setattr(pkg_main, "_nixos_build_env", lambda: env)
...
def main_pkg(monkeypatch, tmp_path):
    ...
    # ``_update_node_dependencies`` calls ``_nixos_build_env()`` (defined
    # inline in the module). On non-NixOS test runs it returns None; we
    # replace it with ``{}`` so ``with_hermes_node_path`` has something to
    # merge without hitting the real NixOS detection code path.
    monkeypatch.setattr(pkg_main, "_nixos_build_env", lambda: {})
    return pkg_main
